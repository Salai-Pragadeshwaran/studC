...
class MainFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        var fab = root.findViewById<FloatingActionButton>(com.example.studc.R.id.sendMessage)
        fab.setOnClickListener {
            var notifText = root.findViewById<EditText>(com.example.studc.R.id.editTextSend)
            var txt: String? = notifText.text.toString()
            notifText.text = null
            if(txt!=null){
                mDatabaseReference.push().setValue(Notification(postText = txt))
            }
            }
